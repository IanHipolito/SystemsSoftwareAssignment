...
static bool store_original_permissions(void) {
    ...
    if (stat(UPLOAD_DIR, &st) == 0) {
        ...
        log_message(LOG_ERROR, "Failed to get permissions for upload directory");
        return false;
    }
    ...
    if (stat(REPORT_DIR, &st) == 0) {
        ...
        log_message(LOG_ERROR, "Failed to get permissions for reporting directory");
        return false;
    }
...
LockStatus lock_directories(void) {
    if (directories_locked) {
        log_message(LOG_WARNING, "Directories already locked");
        return LOCK_SUCCESS;
    }
    ...
    if (chmod(UPLOAD_DIR, 0555) == -1) {
        log_message(LOG_ERROR, "Failed to lock upload directory");
        return LOCK_FAILURE;
    }
    ...
    if (chmod(REPORT_DIR, 0555) == -1) {
        ...
        log_message(LOG_ERROR, "Failed to lock reporting directory");
        return LOCK_FAILURE;
    }
    
    directories_locked = true;
    log_message(LOG_INFO, "Directories locked for backup/transfer");
    return LOCK_SUCCESS;
}

LockStatus unlock_directories(void) {
    if (!directories_locked) {
        log_message(LOG_WARNING, "Directories not locked");
        return UNLOCK_SUCCESS;
    }
    
    // Restore original permissions
    if (chmod(UPLOAD_DIR, upload_orig_mode) == -1) {
        log_message(LOG_ERROR, "Failed to unlock upload directory");
        return UNLOCK_FAILURE;
    }
    
    if (chmod(REPORT_DIR, report_orig_mode) == -1) {
        log_message(LOG_ERROR, "Failed to unlock reporting directory");
        return UNLOCK_FAILURE;
    }
    
    directories_locked = false;
    log_message(LOG_INFO, "Directories unlocked after backup/transfer");
    return UNLOCK_SUCCESS;
}
